...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower().startswith("add "):
        ...
        try:
            ...
            traceback.print_exc()
            reply = "I got the task, bitch, but my Notion brain is acting up."

    # DONE TASK (NEW COMMAND)
        ...
        try:
            ...
            if success:
                reply = f"Nice. Marked '{task}' as done, bitch."
            else:
                reply = f"Couldn't find '{task}' in Notion. You sure it exists?"

        except Exception:
            traceback.print_exc()
            reply = "Something broke while updating Notion, bitch."
...
def jesse_free_reply(text):
    ...
    if "help" in text:
        return "Alright, I'm here. Try: add task or done task."
