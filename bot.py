...
# -------------------------
# NOTION SAFE TASK READER (FIXED ROOT ISSUE)
# -------------------------
def get_tasks():
    try:
        ...
        for r in results["results"]:
            try:
                ...
                # SAFE STATUS EXTRACTION (handles ALL Notion variations)
                status_obj = r["properties"]["Status"].get("select")
...
# -------------------------
# 🔥 FIX: ROBUST STATUS MATCHING (THIS IS THE REAL FIX)
# -------------------------
def normalize(text):
...
def pending_tasks():
    ...
    return [
        t for t in tasks
        if "pending" in normalize(t.get("status"))
    ]
